...
<x-app-layout>
    ...
    <x-form>
    @php
        $emp=DB::table('users')->where('id', (new App\Http\Controllers\Controller)->parse_id(Auth::user()->id) )->first();
        $unit = $emp->unit_id;
        $dep = $emp->dep_id;
        $sec = $emp->sec_id;

        if(isset($sec)){
            $emp=DB::table('users')->where('id', (new App\Http\Controllers\Controller)->parse_id(Auth::user()->id) )
    ...
    @endphp
        ...
        <form action="{{ route('add_emp') }}" method="post" id="sendForm">
            @csrf
                @if( isset($_GET['unit_id']) )
